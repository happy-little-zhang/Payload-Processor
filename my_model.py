...
# 将字节负载值转化成64位2进制字符串的数值
def payload_bytes_to_bin_str(payload_bytes):
    """
    :param payload_bytes:  16进制字节负载，例如：[00, 00, 00, 00, 00, 00, 00, 00]
    :return:                2进制字符串，例如：00000000...00000000
    """
    # 将负载值转化成64位的数值
    payload_bin_str = ""
    ...
    return payload_bin_str


# 将2进制负载字符串按照一定的块大小拆分
# divide the payload into blocks
def payload_bin_str_to_blocks(payload_bin_str, block_size):
    """
    :param payload_bin_str: 2进制负载字符串，例如：00000000...00000000
    :param block_size:      划分的块大小，单位bits
    :return:                以列表形式的块，值为10进制
    """
    # 根据划分的长度拆分负载
    blocks = [int(payload_bin_str[i:i + block_size], 2) for i in range(0, len(payload_bin_str), block_size)]

    return blocks


# 根据块字典压缩单个块
def encode_single_block(block_value, block_dictionary, block_size):
    """
    :param block_value:             块的值，10进制
    :param block_dictionary:        块字典
    :param block_size:              模型划分的块大小
    :return:                        块压缩标记block_bitmap, 编码的块encoded_block(格式均为2进制字符串)
    """

    if block_value not in block_dictionary:
        block_bitmap = "0"  # 无法压缩
        encoded_block = bin(block_value)[2:].zfill(block_size)
    else:
        find_index = block_dictionary.index(block_value)  # 字节值在字典映射中的索引
        bit_len = math.ceil(math.log2(len(block_dictionary)))
        block_bitmap = "1"  # 可以压缩

        if len(block_dictionary) == 1:
            ...
            encoded_block = binary_index

    return block_bitmap, encoded_block


# 判断某个块对应的字典是否只有孤立的-1
def check_single_element_minus_one(my_list):
    """
    Checks if a list contains only one element and that element is -1.

    Args:
        my_list: The list to check.

    Returns:
        True if the list contains only one element with a value of -1, False otherwise.
        Returns None if input is not a list or if list is empty.
    """
    ...
    if not isinstance(my_list, list):
        ...
        return None
    if not my_list:
        return False  # or raise exception, depending on the desired behavior
...
class TBDR:
    def __init__(self, block_size=8, read_ratio=0.5):
        self.dictionary = defaultdict(list)  # 存储映射字典
    ...
    def dictionary_building(self, data_str):
        ...
        for line in lines:
            ...
            self.dictionary_update(m_id, payload_bytes)
    ...
    def message_encoding(self, m_id, m_payload):
        ...
        # 将负载值转化成64位的数值
        payload_bin_str = payload_bytes_to_bin_str(m_payload)

        # 根据划分的长度拆分负载
        blocks_list = payload_bin_str_to_blocks(payload_bin_str, block_size)

        for i, bits_value in enumerate(blocks_list):

            # 1.读取相应的字典映射码
            bits_value_dictionary = dictionary[(m_id, i)]

            block_bitmap, encoded_block = encode_single_block(bits_value, bits_value_dictionary, block_size)

            compression_marker_str += block_bitmap
            encoding_message_str += encoded_block

            list_bit_len.append(len(encoded_block))
    ...
    def message_decoding(self, m_id, message_str):
        ...
        # 根据划分的长度计算位图长度(bits)
        bitmap_length = int(64 / block_size)
        ...
        for i, bit_value in enumerate(bitmap_header):
            if bit_value == "0":
                ...
                if cc_bit_len == 0:
                    ...
                    m_payload_bin_str += bin(cc_bits_value)[2:].zfill(block_size)
                else:
    ...
    def dictionary_update(self, m_id, m_payload):
        ...
        # 根据划分的长度拆分负载
        blocks_list = payload_bin_str_to_blocks(payload_bin_str, self.block_size)

        for i, bits_value in enumerate(blocks_list):

            if bits_value not in self.dictionary[(m_id, i)]:
                # 静态字典动态更新
                self.dictionary[(m_id, i)].append(bits_value)


# improved table based data reduction
class ITBDR:
    def __init__(self, block_size=8, read_ratio=0.5, silence_threshold=5):
        self.dictionary = defaultdict(list)  # 存储映射字典
        self.block_size = block_size  # 64个bit位切分的块大小
        self.read_ratio = read_ratio  # 训练字典的数据比例,取值[0, 1]
    ...
    def message_decoding(self, m_id, message_str):
        ...
        # build a new bitmap, including ignore part
        new_bitmap = ""
        bitmap_header_pos = 0
        for pos in range(block_quantity):
            if pos in ignore_pos:
                ...
                new_bitmap += bitmap_header[bitmap_header_pos]
                bitmap_header_pos = bitmap_header_pos + 1

        for pos in range(block_quantity):
            bit_value = new_bitmap[pos]
            if bit_value == "0":
                # 不可压缩
    ...
    def message_decoding_online(self, m_id, message_str, minimum_mac_len=8):
        """
        根据字典对接收的消息进行解压缩
        :param minimum_mac_len:                  # 最小认证标签长度
        :param m_id:        消息ID
        :param message_str: 编码形式的二进制序列
        :return: 原始消息, MAC
        """
        ...
        for pos in range(block_quantity):
            ...
            if bit_value == "0":
                # 不可压缩
                cc_bit_len = bit_length
                expected_len.append(cc_bit_len)
            else:
                # 可以压缩
                # 1.读取相应的字典映射码
                bits_value_dictionary = dictionary[(m_id, pos)]
        ...
        total_expected_len = bitmap_length + sum(expected_len)

        mac_str = ""

        # complete frame
        if total_expected_len + minimum_mac_len <= 64:
            complete_frame_flag = True
            part_mark_flag = -1

            mac_str = message_str[total_expected_len:]
            ...
            for pos in range(block_quantity):
                ...
                if bit_value == "0":
                    ...
                    # 1.读取相应的字典映射码
                    bits_value_dictionary = dictionary[(m_id, pos)]
                    # print(f"ID: {m_id:x}, pos: {pos}, bits_value_dictionary: {bits_value_dictionary}")
            ...
            # print(f"processing incomplete frame...")

            complete_frame_flag = False
            part_mark_flag = -1

            part_mark = message_str[bitmap_length]
            cc_message_str = cc_message_str[1:]  # 1 bit for partition mark

            if part_mark == "0":
                part_mark_flag = 0
                part1_len = bitmap_length + 1 + sum(expected_len[:len(expected_len) // 2])
                mac_str = message_str[part1_len:]

                # frame part 1
                for pos in range(block_quantity // 2):
                    bit_value = new_bitmap[pos]
                ...
                part_mark_flag = 1
                part2_len = bitmap_length + 1 + sum(expected_len[len(expected_len) // 2:])
                mac_str = message_str[part2_len:]

                # frame part 2
                for pos in range(block_quantity // 2, block_quantity):
                    bit_value = new_bitmap[pos]
        ...
        return complete_frame_flag, part_mark_flag, m_payload, mac_str
    ...
    def dictionary_update(self, m_id, m_payload):
        ...
        payload_bin_str = payload_bytes_to_bin_str(m_payload)
        bits_list = payload_bin_str_to_blocks(payload_bin_str, self.block_size)

        for block_pos, bits_value in enumerate(bits_list):

            if bits_value not in self.dictionary[(m_id, block_pos)]:

                # 判断该位置是否被忽略
                ignore_flag = False
                if len(self.dictionary[(m_id, block_pos)]) == 1:
                    # -1表示该值已经被放弃
                    if self.dictionary[(m_id, block_pos)][0] == -1:
                        ignore_flag = True

                if ignore_flag is False:
                    # 字典动态更新
                    self.dictionary[(m_id, block_pos)].append(bits_value)

                    # 如果取值数目大于(固定值)上限的一半，则没有必要存储了，并用-1标记
                    if len(self.dictionary[(m_id, block_pos)]) > pow(2, self.silence_threshold):
                        self.dictionary[(m_id, block_pos)].clear()
                        self.dictionary[(m_id, block_pos)].append(-1)
    ...
    def message_encoding_with_detailed_output(self, m_id, m_payload):
        ...
        payload_bin_str = payload_bytes_to_bin_str(m_payload)
        bits_list = payload_bin_str_to_blocks(payload_bin_str, block_size)
    ...
    def message_encoding_online(self, m_id, m_payload):
        """
        根据字典对消息进行压缩，同时返回标记位大小、每个块压缩后的大小
        :param dictionary: 字节取值字典
        :param m_id:       消息ID
        ...
        return compression_marker_str, encoding_message_str, list_bit_len, bits_list

    def dictionary_update_online(self, m_id, bits_list):
        """
        根据接收的消息对字典进行更新（使用模型更新，压缩更稳定）
        :param bits_list:  报文负载块对应的10进制值
        :param dictionary: 字节取值字典
        :param m_id:       消息ID
        :param m_payload:  消息负载
        :return:           无
...
# table and delta based data reduction
class TDBDR:
    def __init__(self, block_size=8, read_ratio=0.5):
        self.dictionary = defaultdict(list)  # 存储映射字典
        self.block_size = block_size  # 64个bit位切分的块大小
        self.read_ratio = read_ratio  # 训练字典的数据比例,取值[0, 1]
        self.previous_cache = {}  # 存储前一刻的报文
    ...
    def dictionary_building(self, data_str):
        ...
        lines = data_str.strip().split("\n")
        total_number = len(lines)
        end_pos = int(total_number * self.read_ratio)
        lines = lines[:end_pos]
    ...
    def message_encoding(self, m_id, m_payload):
        ...
        if m_id not in previous_cache:
            ...
            previous_bits_list = previous_cache[m_id]

            for pos in range(len(bits_list)):
                diff = bits_list[pos] - previous_bits_list[pos]
                delta_value = (diff + upper_bound) % upper_bound

                # 1.读取相应的字典映射码
                delta_value_dictionary = dictionary[(m_id, pos)]

                if delta_value not in delta_value_dictionary:
                    bit_len = block_size
                    compression_marker_str += "0"  # 无法压缩
                    encoding_message_str += bin(delta_value)[2:].zfill(bit_len)

                else:
                    find_index = delta_value_dictionary.index(delta_value)  # 字节值在字典映射中的索引
                    bit_len = math.ceil(math.log2(len(delta_value_dictionary)))
                    compression_marker_str += "1"  # 可以压缩

                    if len(delta_value_dictionary) == 1:
                        # 如果字节值取值可能只有1种，不需要占用额外空间
                        encoding_message_str += ""
                    else:
                        # 使用内置的 bin() 函数将 10 进制数转换为二进制字符串。这个函数会在结果前面加上 "0b" 前缀,所以我们使用 [2:] 来去除前缀
    ...
    def message_decoding(self, m_id, message_str):
        ...
        if m_id not in previous_cache:
            ...
            for i, bit_value in enumerate(bitmap_header):
                previous_bit_value = previous_bits_list[i]
                ...
                if bit_value == "0":
                    ...
                    cc_bits_value = cc_message_str[0:cc_bit_len]
                    # print(f"cc_bits_value: {cc_bits_value}, previous_bit_value: {previous_bit_value}")
                    actual_value = (previous_bit_value + int(cc_bits_value, 2)) % upper_bound
                    m_payload_bin_str += bin(actual_value)[2:].zfill(block_size)

                    cc_message_str = cc_message_str[cc_bit_len:]
                else:
                    # 可以压缩
                    # 1.读取相应的字典映射码
                    delta_value_dictionary = dictionary[(m_id, i)]
                    # print(f"ID: {m_id:x}, pos: {i}, bits_value_dictionary: {bits_value_dictionary}")

                    cc_bit_len = math.ceil(math.log2(len(delta_value_dictionary)))
                    bit_len.append(cc_bit_len)

                    if cc_bit_len == 0:
                        # 字典取值数目仅为1
                        cc_bits_value = delta_value_dictionary[0]
                        # print(f"cc_bits_value: {cc_bits_value}, previous_bit_value: {previous_bit_value}")
                        actual_value = (previous_bit_value + cc_bits_value) % upper_bound
                        m_payload_bin_str += bin(actual_value)[2:].zfill(block_size)
                    else:
                        cc_bits_value_index_str = cc_message_str[0:cc_bit_len]
                        # 字典值索引
                        cc_bits_value_index = int(cc_bits_value_index_str, 2)
                        # 字典值
                        cc_bits_value = delta_value_dictionary[cc_bits_value_index]
                        actual_value = (previous_bit_value + cc_bits_value) % upper_bound
                        m_payload_bin_str += bin(actual_value)[2:].zfill(block_size)
    ...
    def dictionary_update(self, m_id, m_payload):
        ...
        if m_id in self.previous_cache:
            upper_bound = pow(2, self.block_size)

            for pos in range(len(bits_list)):
                diff = bits_list[pos] - self.previous_cache[m_id][pos]
                delta_value = (diff + upper_bound) % upper_bound

                if delta_value not in self.dictionary[(m_id, pos)]:
                    # 静态字典动态更新
                    self.dictionary[(m_id, pos)].append(delta_value)

        self.previous_cache[m_id] = bits_list


# improved table and delta based data reduction
class ITDBDR:
    def __init__(self, block_size=8, read_ratio=0.5, silence_threshold=5):
        self.dictionary = defaultdict(list)  # 存储映射字典
        self.block_size = block_size  # 64个bit位切分的块大小
        ...
        self.silence_threshold = silence_threshold  # 优化时活跃值数目所占位数的阈值
        self.previous_cache = {}  # 存储前一刻的报文
        self.upper_bound = pow(2, block_size)
    ...
    def message_decoding(self, m_id, message_str):
        ...
        # 计算块数目
        block_quantity = int(64 / block_size)
        ...
        # 第一帧，没有缓存
        if m_id not in previous_cache:
            bitmap_header = message_str[0:block_quantity]
            cc_message_str = message_str[block_quantity:]
            ...
            for i, bit_value in enumerate(new_bitmap):
                previous_bit_value = previous_bits_list[i]
    ...
    def dictionary_update(self, m_id, m_payload):
        ...
        if m_id in self.previous_cache:
            upper_bound = pow(2, self.block_size)
            previous_bits_list = self.previous_cache[m_id]
            ...
            for pos in range(len(bits_list)):
                ...
                if delta_value not in self.dictionary[(m_id, pos)]:
                    ...
                    if not check_single_element_minus_one(self.dictionary[(m_id, pos)]):
                        # 字典动态更新
                        self.dictionary[(m_id, pos)].append(delta_value)

                        # 如果取值数目大于(固定值)上限的一半，则没有必要存储了，并用-1标记
                        if len(self.dictionary[(m_id, pos)]) > pow(2, self.silence_threshold):
                            self.dictionary[(m_id, pos)].clear()
                            self.dictionary[(m_id, pos)].append(-1)
    ...
    def message_encoding_with_detailed_output(self, m_id, m_payload):
        ...
        # 根据划分的长度拆分负载
        bits_list = payload_bin_str_to_blocks(payload_bin_str, bit_length)
...
# value and difference combination + table based data reduction
class VDCTBDR:
    def __init__(self, block_size=8, read_ratio=0.5):
        self.value_dictionary = defaultdict(list)        # 存储值映射字典     例如：a[(0x123, 1) = [0, 1, 2, 3]]
        self.difference_dictionary = defaultdict(list)   # 存储差分映射字典    例如：a[(0x123, 1) = [0, 1, 2, 3]]
    ...
    def message_encoding(self, m_id, m_payload):
        ...
        difference_dictionary = self.difference_dictionary
        block_size = self.block_size
        previous_cache = self.previous_cache
        ...
        if m_id not in previous_cache:
            for i, bits_value in enumerate(blocks_list):
                # 1.读取相应的字典映射码
                block_value_dictionary = value_dictionary[(m_id, i)]
                if bits_value not in block_value_dictionary:
                    bit_len = block_size
                    compression_marker_str += "0"  # 无法压缩
                    encoding_message_str += bin(bits_value)[2:].zfill(bit_len)

                else:
                    find_index = block_value_dictionary.index(bits_value)  # 字节值在字典映射中的索引
                    bit_len = math.ceil(math.log2(len(block_value_dictionary)))
                    compression_marker_str += "1"  # 可以压缩

                    if len(block_value_dictionary) == 1:
                        # 如果字节值取值可能只有1种，不需要占用额外空间
                        encoding_message_str += ""
                    else:
                        # 使用内置的 bin() 函数将 10 进制数转换为二进制字符串。这个函数会在结果前面加上 "0b" 前缀,所以我们使用 [2:] 来去除前缀
            ...
            for pos in range(len(blocks_list)):
                # 1.读取相应的字典映射码
                block_value_dictionary = value_dictionary[(m_id, pos)]
                block_difference_dictionary = difference_dictionary[(m_id, pos)]

                if len(block_value_dictionary) <= len(block_difference_dictionary):
                    block_value = blocks_list[pos]
                    if block_value not in block_value_dictionary:
                        bit_len = block_size
                        compression_marker_str += "0"  # 无法压缩
                        encoding_message_str += bin(block_value)[2:].zfill(bit_len)

                    else:
                        find_index = block_value_dictionary.index(block_value)  # 字节值在字典映射中的索引
                        bit_len = math.ceil(math.log2(len(block_value_dictionary)))
                        compression_marker_str += "1"  # 可以压缩
                    ...
                    diff = blocks_list[pos] - previous_cache[m_id][pos]
                    delta_value = (diff + upper_bound) % upper_bound
                    if delta_value not in block_difference_dictionary:
                        bit_len = block_size
                        compression_marker_str += "0"  # 无法压缩
                        encoding_message_str += bin(delta_value)[2:].zfill(bit_len)

                    else:
                        find_index = block_difference_dictionary.index(delta_value)  # 字节值在字典映射中的索引
                        bit_len = math.ceil(math.log2(len(block_difference_dictionary)))
                        compression_marker_str += "1"  # 可以压缩

                        if len(block_difference_dictionary) == 1:
                            # 如果字节值取值可能只有1种，不需要占用额外空间
                            encoding_message_str += ""
                        else:
                            # 使用内置的 bin() 函数将 10 进制数转换为二进制字符串。这个函数会在结果前面加上 "0b" 前缀,所以我们使用 [2:] 来去除前缀
        ...
        if m_id not in previous_cache:
            for i, bit_value in enumerate(bitmap_header):
                if bit_value == "0":
                    ...
                    cc_bit_len = math.ceil(math.log2(len(block_value_dictionary)))
                    bit_len.append(cc_bit_len)

                    if cc_bit_len == 0:
                        # 字典取值数目仅为1
                        cc_bits_value = block_value_dictionary[0]
                        m_payload_bin_str += bin(cc_bits_value)[2:].zfill(block_size)
                    else:
                        cc_bits_value_index_str = cc_message_str[0:cc_bit_len]
                        # 字典值索引
                        cc_bits_value_index = int(cc_bits_value_index_str, 2)
                        # 字典值
                        cc_bits_value = block_value_dictionary[cc_bits_value_index]
                        m_payload_bin_str += bin(cc_bits_value)[2:].zfill(block_size)
            ...
            for pos in range(bitmap_length):
                # 1.读取相应的字典映射码
                block_value_dictionary = value_dictionary[(m_id, pos)]
                block_difference_dictionary = difference_dictionary[(m_id, pos)]

                bit_value = bitmap_header[pos]
                ...
                if len(block_value_dictionary) <= len(block_difference_dictionary):
                    ...
                    previous_bit_value = previous_bits_list[pos]
                    ...
                    if bit_value == "0":
                        ...
                        # 可以压缩
                        cc_bit_len = math.ceil(math.log2(len(block_difference_dictionary)))
                        bit_len.append(cc_bit_len)

                        if cc_bit_len == 0:
                            # 字典取值数目仅为1
                            cc_bits_value = block_difference_dictionary[0]
                            # print(f"cc_bits_value: {cc_bits_value}, previous_bit_value: {previous_bit_value}")
                            actual_value = (previous_bit_value + cc_bits_value) % upper_bound
                            m_payload_bin_str += bin(actual_value)[2:].zfill(block_size)
                        else:
                            ...
                            cc_bits_value_index = int(cc_bits_value_index_str, 2)
                            # 字典值
                            cc_bits_value = block_difference_dictionary[cc_bits_value_index]
                            actual_value = (previous_bit_value + cc_bits_value) % upper_bound
                            m_payload_bin_str += bin(actual_value)[2:].zfill(block_size)
    ...
    def dictionary_update(self, m_id, m_payload):
        ...
        if m_id in self.previous_cache:
            upper_bound = pow(2, self.block_size)
            previous_blocks_list = self.previous_cache[m_id]

            for pos in range(len(blocks_list)):
                diff = blocks_list[pos] - previous_blocks_list[pos]
                diff = (diff + upper_bound) % upper_bound

                if diff not in self.difference_dictionary[(m_id, pos)]:
                    # 静态字典动态更新
                    self.difference_dictionary[(m_id, pos)].append(diff)

        self.previous_cache[m_id] = blocks_list

        for pos in range(len(blocks_list)):
            block_value = blocks_list[pos]
            if block_value not in self.value_dictionary[(m_id, pos)]:
                # 静态字典动态更新
                self.value_dictionary[(m_id, pos)].append(block_value)


# improved value and difference combination + table based data reduction
class IVDCTBDR:
    def __init__(self, block_size=8, read_ratio=0.5, silence_threshold=5):
        self.value_dictionary = defaultdict(list)        # 存储值映射字典     例如：a[(0x123, 1) = [0, 1, 2, 3]]
        self.difference_dictionary = defaultdict(list)   # 存储差分映射字典    例如：a[(0x123, 1) = [0, 1, 2, 3]]
    ...
    def message_encoding(self, m_id, m_payload):
        ...
        for pos in range(len(blocks_list)):
            if m_id not in self.previous_cache:
                selected_encoding_option.append(1)
            else:
                # 1.读取相应的字典映射码
                block_value_dictionary = self.value_dictionary[(m_id, pos)]
                block_difference_dictionary = self.difference_dictionary[(m_id, pos)]

                select_option = 1
                if not check_single_element_minus_one(block_value_dictionary) and not check_single_element_minus_one(block_difference_dictionary):
                    if len(block_value_dictionary) <= len(block_difference_dictionary):
                        select_option = 1
                    else:
                        select_option = 2
                elif check_single_element_minus_one(block_value_dictionary) and not check_single_element_minus_one(block_difference_dictionary):
                    select_option = 2
                elif not check_single_element_minus_one(block_value_dictionary) and check_single_element_minus_one(block_difference_dictionary):
                    select_option = 1
                else:
                    select_option = 1
                selected_encoding_option.append(select_option)

        # 开始编码
        for pos in range(len(blocks_list)):
            if selected_encoding_option[pos] == 1:
                bits_value = blocks_list[pos]
                # 1.读取相应的字典映射码
                block_value_dictionary = self.value_dictionary[(m_id, pos)]
                if bits_value not in block_value_dictionary:
                    bit_len = self.block_size

                    if check_single_element_minus_one(block_value_dictionary):
                        compression_marker_str += ""  # 无法压缩,且压缩标记被忽略
                    else:
                        compression_marker_str += "0"  # 无法压缩
                    encoding_message_str += bin(bits_value)[2:].zfill(bit_len)
                ...
                diff = blocks_list[pos] - self.previous_cache[m_id][pos]
                delta_value = (diff + upper_bound) % upper_bound
                ...
                if delta_value not in block_difference_dictionary:
                    ...
                    if check_single_element_minus_one(block_difference_dictionary):
                        compression_marker_str += ""  # 无法压缩,且压缩标记被忽略
                    else:
                        compression_marker_str += "0"  # 无法压缩
                    encoding_message_str += bin(delta_value)[2:].zfill(bit_len)
    ...
    def message_decoding(self, m_id, message_str):
        ...
        # 根据划分的长度计算位图长度(bits)
        block_quantity = int(64 / self.block_size)
        ...
        for pos in range(block_quantity):
            ...
            if bit_value == "0":
                ...
                if selected_encoding_option[pos] == 1:
                    ...
                    previous_bit_value = self.previous_cache[m_id][pos]
                    actual_value = (previous_bit_value + int(cc_bits_value, 2)) % upper_bound
                    m_payload_bin_str += bin(actual_value)[2:].zfill(self.block_size)
                ...
                if selected_encoding_option[pos] == 1:
                    ...
                    if cc_bit_len == 0:
                        # 字典取值数目仅为1
                        cc_bits_value = block_value_dictionary[0]
                        m_payload_bin_str += bin(cc_bits_value)[2:].zfill(self.block_size)
                    else:
                        cc_bits_value_index_str = cc_message_str[0:cc_bit_len]
                        # 字典值索引
                        cc_bits_value_index = int(cc_bits_value_index_str, 2)
    ...
    def dictionary_update(self, m_id, m_payload):
        ...
        if m_id in self.previous_cache:
            ...
            for pos in range(len(blocks_list)):
                ...
                if not check_single_element_minus_one(self.difference_dictionary[(m_id, pos)]):

                    if diff not in self.difference_dictionary[(m_id, pos)]:
                        # 静态字典动态更新
                        self.difference_dictionary[(m_id, pos)].append(diff)

                        if len(self.difference_dictionary[(m_id, pos)]) > pow(2, self.silence_threshold):
                            self.difference_dictionary[(m_id, pos)].clear()
                            self.difference_dictionary[(m_id, pos)].append(-1)
        ...
        for pos in range(len(blocks_list)):
            ...
            if not check_single_element_minus_one(self.value_dictionary[(m_id, pos)]):

                if block_value not in self.value_dictionary[(m_id, pos)]:
                    # 静态字典动态更新
                    self.value_dictionary[(m_id, pos)].append(block_value)

                    if len(self.value_dictionary[(m_id, pos)]) > pow(2, self.silence_threshold):
                        self.value_dictionary[(m_id, pos)].clear()
                        self.value_dictionary[(m_id, pos)].append(-1)
    ...
    def message_encoding_online(self, m_id, m_payload):
        ...
        return compression_marker_str, encoding_message_str, list_bit_len
...
class SFDC:
    def __init__(self, read_ratio=0.5):
    ...
    def message_encoding(self, m_id, m_payload):
        ...
        # 当前ID是新出现的，不存在矩阵
            ...
            # 从后向前遍历字符串,找到第一个非零字符的位置
            last_pos_index = -1
            for i in range(len(payload_bin_str) - 1, -1, -1):
                if payload_bin_str[i] != "0":
                    last_pos_index = i
                    break

            message_str = ""
            if last_pos_index >= 0:
                message_str = "" + payload_bin_str[:last_pos_index + 1]
    ...
    def message_decoding(self, m_id, message_str):
        ...
        if m_id in inversion_pos:
            ...
            m_payload_bin_str = ''.join(payload_bin_list)
